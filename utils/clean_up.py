...
LABEL_WORDS = [
    "ANDR", "ÄNDR",
    "ANLÄGGNINGSTYP", "ANLAGGNINGSTYP","ANLAGENINGSTYP",
    "AVDELNING",
    "BANDEL",
    "BLAD",
    "BESKRIVNING",
...
def clean_name_field(val):
    ...
    text = re.sub(r"^[ECO]RANSKAD", "GRANSKAD", text, flags=re.IGNORECASE)
...
def clean_beskrivning_3(val):
    ...
    garbage_patterns = [
        r"\bU[NnVvIi\|]{2,}\b",   # Unui, Uuii, U|ui, Unni, etc
        r"\bUNU[I1\|]\b",        # UNU1, UNU|, UNUi
        r"^\W+$",                # Only symbols like '|||' or '--'
    ]

    for gp in garbage_patterns:
        t = re.sub(gp, "", t, flags=re.IGNORECASE)

    # Remove isolated 1–2 letter garbage words (rare case)
    t = re.sub(r"\b[A-Z]{1,2}\b", "", t)

    # Normalize the internal pipe separator
    t = re.sub(r"\s*\|\s*", " | ", t)
...
def clean_granskad_av(val):
    ...
    t = str(val).strip()
    t = re.sub(r"^[ECO]RANSKAD", "GRANSKAD", t, flags=re.IGNORECASE)
...
# ================================================================
#  Fixed  RULES to keep these columns with predefined values
# ================================================================
...
# ================================================================
#  MAIN CLEAN FUNCTION (useful for streamlit to call)
# ================================================================
